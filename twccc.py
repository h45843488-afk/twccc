    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for row in data:
                ...
                info_meta = stocks_map[code]
                sector = str(info_meta.get('sector', '未分類')).strip() or '未分類'
                ...
                try:
                    price = float(row.get('ClosingPrice', 0))
                    vol_lots = int(float(row.get('TradeVolume', 0)) / 1000)
                    high = float(row.get('HighestPrice', 0))
                    low = float(row.get('LowestPrice', 0))
                    change = float(row.get('Change', 0))
...
# =============================================================================
# 🛠️ 輔助函式：從證交所官方 API 抓取歷史數據
# =============================================================================
def get_twse_stock_data(code):
    """從證交所官方 API 抓取近幾個月的每日交易數據."""
    headers = {'User-Agent': 'Mozilla/5.0'}
    session = requests.Session()

    today = pd.Timestamp.now()
    months = [today - pd.DateOffset(months=i) for i in range(4)]
        ...
        try:
            ...
            if data.get('stat') == 'OK' and 'data' in data:
                for row in data['data']:
                    ...
                    ad_year = int(year_str) + 1911
                    dt = pd.to_datetime(f'{ad_year}-{month_str}-{day_str}')

                    def parse_val(val):
                        return float(str(val).replace(',', '')) if val != '--' else np.nan

                    open_p = parse_val(row[3])
                    high_p = parse_val(row[4])
                    low_p = parse_val(row[5])
                    close_p = parse_val(row[6])
                    vol = parse_val(row[1])
    ...
    df = pd.DataFrame(all_data).drop_duplicates(subset=['Date'])
    df = df.sort_values('Date').reset_index(drop=True)
    return df


# =============================================================================
# 🔥 【第三關】趨勢紅線 + 轉折訊號過濾 (短線進階版)
# =============================================================================
def run_stage3_filter(candidates):
    print("==============================================")
    print("🔥 開始進入第三關：【趨勢紅線 + 轉折訊號過濾】...")
    print("==============================================\n")
    ...
    for code, info in candidates.items():
        ...
        try:
            ...
            # 紅綠線狀態 (ZYG29 > ZYG30 為紅線)
            df["ZYG_Red"] = df["ZYG29"] > df["ZYG30"]
            ...
            # --- 判斷條件 ---
            is_red = bool(df["ZYG_Red"].iloc[-1])
            recent_cross = df["ZYG_CROSS_BUY"].tail(3).values
            has_signal = any(recent_cross)

            if is_red and has_signal:
                bias_ma20 = ((price - latest_ma20) / latest_ma20) * 100 if latest_ma20 > 0 else 999.0
                
                sig_text = "紅線轉折"
                if df["HIGH_WIN_BUY"].tail(3).any():
                    sig_text = "突破"
                elif df["BASE_GD"].tail(3).any():
                    sig_text = "轉折"

                info['bias_ma20'] = round(bias_ma20, 2)
                info['signal_text'] = sig_text
                stage3_targets[code] = info
                
                print(f"👉 {code} {info['name']} -> ✅ 第三關通過 [訊號: {sig_text} | 乖離: {bias_ma20:+.2f}%]")
...
def run_stage5_risk_management_and_wrapup(stage4_targets):
    ...
    summary_list = []
    msg_lines = ["📊 【短線吃肉選股戰情室】", f"🎯 通過最終篩選標的共 {len(stage4_targets)} 檔：\n"]
